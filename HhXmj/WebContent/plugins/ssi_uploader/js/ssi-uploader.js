...
(function (root, factory) {
    ...
    Ssi_upload.prototype.init = function (element) {
        ...
        var $uploadBtn = '';
        if(this.options.formDom){
        	$uploadBtn = $('<button id="ssi-uploadBtn" hidden class="ssi-button success ssi-hidden" >' +
        	         '<span class="ssi-btnIn">' + this.language.upload + '&nbsp;</span>' +
        	         '<div id="ssi-up_loading" class="ssi-btnIn"></div></button>');
        }else{
        	$uploadBtn = $('<button id="ssi-uploadBtn" class="ssi-button success ssi-hidden" >' +
       	         '<span class="ssi-btnIn">' + this.language.upload + '&nbsp;</span>' +
       	         '<div id="ssi-up_loading" class="ssi-btnIn"></div></button>');
...
//----------------------------UPLOADFILES------------------------------------
    ...
    Ssi_upload.prototype.uploadFiles = function () {// upload the pending files
        if (this.pending > 0) {
            ...
            if(this.options.formDom){
            	formData.append("fid", this.options.formDom.val());
            }
    ...
    var locale = {
        en: {
            ...
            extError: '$1 不支持该文件类型',//$1=file extension ie(exe files are not supported)
