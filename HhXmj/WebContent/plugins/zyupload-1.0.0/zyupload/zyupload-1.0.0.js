...
var ZYFILE = {
	...
	funUploadFile : function(file){
		...
		$.each(self.data, function (key, value) {// append all extra data
...
//					edit             : true,  						      // 是否可以编辑文件（裁剪）
			...
			this.init = function(){
				this.createHtml();  // 创建组件html
				this.createCorePlug(para);  // 调用核心js
			};
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
				    ...
				    html += '							<img class="upload_image" src="plugins/zyupload-1.0.0/zyupload/skins/images/add_img.png" style="width:expression(this.width > '+imgWidth+' ? '+imgWidth+'px : this.width)" />';
				    html += '						</div>'; 
				    html += '					</a>';
				    html += '				</div>';
					html += '			</div>';
			...
			this.funDisposePreviewHtml = function(file, e){
				...
				var type = newStr.split(".")[0].split("").reverse().join("");
				// 处理不同类型文件代表的图标
				var fileImgSrc = "plugins/zyupload-1.0.0/zyupload/skins/images/fileType/";
				if(type == "rar"){
...
//						});
			 ...
			 * 返回: 无
			 */
			this.createCorePlug = function(param){
				var params = {
