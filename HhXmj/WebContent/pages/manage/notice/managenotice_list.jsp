...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    <section class="content">
      <div class="row">
        <div class="col-xs-12">
          <form class="form-horizontal" action="manage/notice/list" name="managenoticeListForm" id="managenoticeListForm" method="post">
        	  <input type="hidden" name="pageIndex" id="pageIndex" value="${manageNotice.pageIndex}"/>
			  <input type="hidden" name="pageSize" id="pageSize" value="${manageNotice.pageSize}"/>
			  <input type="hidden" name="total" id="total" value="${manageNotice.total}"/>
	                ...
	                <div class="form-group">
	                  ...
	                  <div class="col-sm-3">
	                    <select id="conditionName" name="conditionName" class="form-control select2" >
									...
									<option value="manage_notice_id" <c:if test="${conditionName=='manage_notice_id'}"> selected </c:if> ></option>
									...
									<option value="manage_notice_createuid" <c:if test="${conditionName=='manage_notice_createuid'}"> selected </c:if> >创建者</option>
									<option value="manage_notice_createtime" <c:if test="${conditionName=='manage_notice_createtime'}"> selected </c:if> >创建时间</option>
									<option value="manage_notice_updatetime" <c:if test="${conditionName=='manage_notice_updatetime'}"> selected </c:if> >更新时间</option>
		                </select>
	                  </div>
	                ...
	                <c:choose>
							<c:when test="${not empty varList}">
								<c:forEach items="${varList}" var="var" varStatus="vs">
									<tr>
											<td>
												<input name='manageNoticeIds' id='manageNoticeIds' type="checkbox" class="flat-red" value="${var.manage_notice_id}" >
											</td>
										    <td>${var.title}</td>
										    <td>${var.sys_user_name}</td>
							              	<td><fmt:formatDate value="${var.manage_notice_createtime}" pattern="yyyy-MM-dd HH:mm:ss" /></td>
							              	<td><fmt:formatDate value="${var.manage_notice_updatetime}" pattern="yyyy-MM-dd HH:mm:ss" /></td>
											<td>
												<shiro:hasPermission name="sysUser:goedit">
													<a style="cursor:pointer;" title="编辑" onclick="edit('manage/notice/goedit?manage_notice_id='+'${var.manage_notice_id}',800);" class="btn btn-social-icon btn-info btn-xs"><i class="fa fa-edit"></i></a>
												</shiro:hasPermission>
												<shiro:hasPermission name="sysUser:del">
													<a style="cursor:pointer;" title="删除" onclick="del('manage/notice/del?manage_notice_id='+'${var.manage_notice_id}',800);" class="btn btn-social-icon btn-primary btn-xs"><i class="fa fa-trash"></i></a>
												</shiro:hasPermission>
											</td>
									</tr>
								</c:forEach>
	            ...
	            <div class="box-footer clearfix">
	            	<div class="pull-left">
	            		<shiro:hasPermission name="sysUser:goadd">
	            			 <a style="cursor:pointer;" title="新增" onclick="add('manage/notice/goadd',800);" class="btn btn-social-icon btn-info"><i class="fa fa-plus-square"></i></a>
	            		</shiro:hasPermission>
	            		<shiro:hasPermission name="sysUser:delall">
	            			<a style="cursor:pointer;" title="删除" onclick="delArray('manage/notice/delall',800);" class="btn btn-social-icon btn-primary"><i class="fa fa-trash"></i></a>
	            		</shiro:hasPermission>
	            	</div>
  ...
  $(function () {
	...
	//Flat red color scheme for iCheck
	 ...
	 $('#checkAll').on('ifChecked', function(event){
	 		$('input#manageNoticeIds').iCheck('check');
	 	});
	 	$('#checkAll').on('ifUnchecked', function(event){
	 		$('input#manageNoticeIds').iCheck('uncheck');
	 	});
	 $("#page").pagination({
    ...
    function delArray(url,height){
 		var manageNoticeIds = new Array();
 		 $("#manageNoticeIds:checked").each(function() {
 			manageNoticeIds.push($(this).val()); 
 		 });
    	window.top.delArrayDialog(url,manageNoticeIds);
    }
    function postObjs(url,height){
 		var objs = new Array();
 		 $("#manageNoticeIds:checked").each(function() {
 			var obj = {};
 			obj.manageNotice_id = $(this).val();
 			objs.push(obj); 
 		 });
