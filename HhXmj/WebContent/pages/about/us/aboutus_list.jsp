...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    <section class="content">
      <div class="row">
        <div class="col-xs-12">
          <form class="form-horizontal" action="about/us/list" name="aboutusListForm" id="aboutusListForm" method="post">
        	  <input type="hidden" name="pageIndex" id="pageIndex" value="${aboutUs.pageIndex}"/>
			  <input type="hidden" name="pageSize" id="pageSize" value="${aboutUs.pageSize}"/>
			  <input type="hidden" name="total" id="total" value="${aboutUs.total}"/>
	                ...
	                <div class="form-group">
	                  ...
	                  <div class="col-sm-3">
	                    <select id="conditionName" name="conditionName" class="form-control select2" >
									...
									<option value="about_us_id" <c:if test="${conditionName=='about_us_id'}"> selected </c:if> ></option>
									...
									<option value="about_us_createuid" <c:if test="${conditionName=='about_us_createuid'}"> selected </c:if> >创建者</option>
									<option value="about_us_createtime" <c:if test="${conditionName=='about_us_createtime'}"> selected </c:if> >创建时间</option>
									<option value="about_us_updatetime" <c:if test="${conditionName=='about_us_updatetime'}"> selected </c:if> >更新时间</option>
		                </select>
	                  </div>
	                ...
	                <c:choose>
							<c:when test="${not empty varList}">
								<c:forEach items="${varList}" var="var" varStatus="vs">
									<tr>
											<td>
												<input name='aboutUsIds' id='aboutUsIds' type="checkbox" class="flat-red" value="${var.about_us_id}" >
											</td>
										    <td>${var.title}</td>
										    <td>${var.sys_user_name}</td>
							              	<td><fmt:formatDate value="${var.about_us_createtime}" pattern="yyyy-MM-dd HH:mm:ss" /></td>
							              	<td><fmt:formatDate value="${var.about_us_updatetime}" pattern="yyyy-MM-dd HH:mm:ss" /></td>
											<td>
												<shiro:hasPermission name="sysUser:goedit">
													<a style="cursor:pointer;" title="编辑" onclick="edit('about/us/goedit?about_us_id='+'${var.about_us_id}',800);" class="btn btn-social-icon btn-info btn-xs"><i class="fa fa-edit"></i></a>
												</shiro:hasPermission>
												<shiro:hasPermission name="sysUser:del">
													<a style="cursor:pointer;" title="删除" onclick="del('about/us/del?about_us_id='+'${var.about_us_id}',800);" class="btn btn-social-icon btn-primary btn-xs"><i class="fa fa-trash"></i></a>
												</shiro:hasPermission>
											</td>
									</tr>
								</c:forEach>
	            ...
	            <div class="box-footer clearfix">
	            	<div class="pull-left">
	            		<shiro:hasPermission name="sysUser:goadd">
	            			 <a style="cursor:pointer;" title="新增" onclick="add('about/us/goadd',800);" class="btn btn-social-icon btn-info"><i class="fa fa-plus-square"></i></a>
	            		</shiro:hasPermission>
	            		<shiro:hasPermission name="sysUser:delall">
	            			<a style="cursor:pointer;" title="删除" onclick="delArray('about/us/delall',800);" class="btn btn-social-icon btn-primary"><i class="fa fa-trash"></i></a>
	            		</shiro:hasPermission>
	            	</div>
  ...
  $(function () {
	...
	//Flat red color scheme for iCheck
	 ...
	 $('#checkAll').on('ifChecked', function(event){
	 		$('input#aboutUsIds').iCheck('check');
	 	});
	 	$('#checkAll').on('ifUnchecked', function(event){
	 		$('input#aboutUsIds').iCheck('uncheck');
	 	});
	 $("#page").pagination({
    ...
    function delArray(url,height){
 		var aboutUsIds = new Array();
 		 $("#aboutUsIds:checked").each(function() {
 			aboutUsIds.push($(this).val()); 
 		 });
    	window.top.delArrayDialog(url,aboutUsIds);
    }
    function postObjs(url,height){
 		var objs = new Array();
 		 $("#aboutUsIds:checked").each(function() {
 			var obj = {};
 			obj.aboutUs_id = $(this).val();
 			objs.push(obj); 
 		 });
