...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    <section class="content">
      <div class="row">
        <div class="col-xs-12">
          <form class="form-horizontal" action="sys/file/list" name="sysfileListForm" id="sysfileListForm" method="post">
        	  <input type="hidden" name="pageIndex" id="pageIndex" value="${sysFile.pageIndex}"/>
			  <input type="hidden" name="pageSize" id="pageSize" value="${sysFile.pageSize}"/>
			  <input type="hidden" name="total" id="total" value="${sysFile.total}"/>
	                ...
	                <div class="form-group">
	                  ...
	                  <div class="col-sm-3">
	                    <select id="conditionName" name="conditionName" class="form-control select2" >
									...
									<option value="sys_file_id" <c:if test="${conditionName=='sys_file_id'}"> selected </c:if> > 主键</option>
									<option value="sys_file_name" <c:if test="${conditionName=='sys_file_name'}"> selected </c:if> >文件名</option>
									<option value="sys_file_addr" <c:if test="${conditionName=='sys_file_addr'}"> selected </c:if> >文件地址</option>
									<option value="sys_file_updatedate" <c:if test="${conditionName=='sys_file_updatedate'}"> selected </c:if> >更新时间</option>
									<option value="sys_file_createdate" <c:if test="${conditionName=='sys_file_createdate'}"> selected </c:if> >创建时间</option>
									<option value="sys_file_updateuid" <c:if test="${conditionName=='sys_file_updateuid'}"> selected </c:if> >修改人</option>
									<option value="sys_file_createuid" <c:if test="${conditionName=='sys_file_createuid'}"> selected </c:if> >创建人</option>
		                </select>
	                  </div>
	                ...
	                <c:choose>
							<c:when test="${not empty varList}">
								<c:forEach items="${varList}" var="var" varStatus="vs">
									<tr>
											<td>
												<input name='sysFileIds' id='sysFileIds' type="checkbox" class="flat-red" value="${var.sys_file_id}" >
											</td>
										    <td>${var.sys_file_name}</td>
										    <td>${var.sys_file_addr}</td>
							              	<td><fmt:formatDate value="${var.sys_file_updatedate}" pattern="yyyy-MM-dd HH:mm:ss" /></td>
										    <td>${var.sys_file_createdate}</td>
										    <td>${var.sys_file_updateuid}</td>
										    <td>${var.sys_file_createuid}</td>
											<td>
												<shiro:hasPermission name="sysFile:download">
													<a style="cursor:pointer;" title="下载" href="sys/file/download?sys_file_name=${var.sys_file_name}" class="btn btn-social-icon btn-primary btn-xs"><i class="fa fa-download"></i></a>
												</shiro:hasPermission>
											</td>
									</tr>
								</c:forEach>
	            ...
	            <div class="box-footer clearfix">
	            	<div class="pull-left">
	            		<shiro:hasPermission name="sysFile:upload">
	            			<a style="cursor:pointer;" title="上传" onclick="upload('sys/file/upload',400);" class="btn btn-social-icon btn-primary"><i class="fa fa-upload"></i></a>
	            		</shiro:hasPermission>
	            	</div>
