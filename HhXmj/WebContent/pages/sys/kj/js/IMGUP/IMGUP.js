...
var IMG_AJAXPATH = "bond/imgup/save";//异步传输服务端位置
...
$("#div_imgfile").click(function () {
    ...
    if ($(".imgfile").length <= $(".lookimg").length) {//个数不足则新创建对象
        ...
        _CRE_FILE.setAttribute("display","none");
        //在修改时，将空缺的编号填充
        var nums = new Array();
        var obj = $(".lookimg");
		for(var i in obj){
		...
		//console.log(UP_IMGCOUNT+","+nums.contains(UP_IMGCOUNT));
		while(nums.contains(UP_IMGCOUNT)){
			UP_IMGCOUNT++;
		}
...
$("#btn_ImgUpStart").click(function () {
    ...
    for (var i = 0; i < $(".lookimg").length; i++) {
        ...
        var IMG_ROUTE1 = $(".imgfile[num=" + IMG_IND + "]").eq(0).val();//获取上传图片路径，为获取图片类型使用
        var IMG_ROUTE =""+IMG_ROUTE1;
        ...
        $.ajax({
            type: "POST",
            url: IMG_AJAXPATH,
            data: { 'imgBase': IMG_BASE, 'imgFormat': IMG_FOMATE, 'lookIndex': IMG_IND },//图片base64编码，图片格式（当前仅支持jpg,png,jpeg三种），图片对象索引
            dataType: "json",
            success: function (data) {
            	debugger
                if (data.resultCode == "1") {
                    //图片上传成功回调
                	console.log("是第几个元素:"+data.ind);
                    ...
                    $(".lookimg").eq([data.ind]).children(".lookimg_progress").eq(0).children("div").eq(0).css("width", "100%").css("background-color", "red").text("上传失败");
                }
            },
            error: function (err) {
                //服务器连接失败报错处理
