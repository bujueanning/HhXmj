			...
			<div class="login_box">
				<div class="regbox-wrap">
					...
					<form id="regForm" name="regForm" action="sys/login/savereg" method="post" >					
						...
						<tbody>
							<tr>   
								<th valign="top" nowrap="nowrap" style="width:22%;" class="tt">名称：</th>
								<td>
								<input id="sys_user_id" name="sys_user_id" type="hidden" value=""></input>
								<input id="sys_user_name" name="sys_user_name" placeholder="请输入10位以内不含特殊字符的名称" onblur="checkName();" class="inpt w200" maxlength="10" style="width:100;" type="text"><span style="color:red">*&nbsp;</span><span id="partFullName_required" style="display:none;color:red;">不能为空</span><span id="partFullName_maxlength" style="display:none;color:red;">机构全称长度超长</span><span id="partFullName_special" style="display:none;color:red;">用户名不能为空或含有特殊字符</span>								
								</td>             
							</tr>
							<tr>
								<th valign="top" nowrap="nowrap" class="tt" style="    height: 20px;">&nbsp;</th>
								<td style="text-align: left;">
									<input id="sys_user_reg_type_d1" name="sys_user_reg_type_d" type="radio" value="a77fda4f171f45f1b089f0dd18221e22" style="float: none;"/>发布方
									<input id="sys_user_reg_type_d" name="sys_user_reg_type_d" type="radio" value="37b73a43f23a42edb6bef45a96628217" style="float: none;"/>处置方 
								</td>	
							</tr>
							<tr>
								<th valign="top" nowrap="nowrap" style="width:22%;" class="tt">登录密码：</th>
								<td>
								<input id="sys_user_password" name="sys_user_password" onblur="checkPass();" placeholder="6-12位数字与字母的组合"  class="inpt w200" maxlength="12" style="width:64" type="password" onselectstart="return false" oncopy="return false" onpaste="return false"><span style="color:red">*&nbsp;</span><span id="password_required" style="display:none;color:red;">登录密码不能为空</span><span id="password_check" style="display:none;color:red;">请输入6~16位字符，至少包含数字、字母（区分大小写）、符号中的2种。</span><span id="password_maxlength" style="display:none;color:red;">登录密码长度超长</span></td></tr><tr><th>确认登录密码：</th><td><input id="confirm_password" name="confirm_password"  onblur="confirmPass();" placeholder="确认登录密码" class="inpt w200" maxlength="16" style="width:64" onblur="checkPwd('password','confirm_password')" type="password" onselectstart="return false" oncopy="return false" onpaste="return false"><span style="color:red">*&nbsp;</span><span id="confirm_password_check" style="display:none;color:red;">两次输入的密码不相同</span>							
								</td>             
							</tr>
							...
							<tr>      
								...
								<td>
									<input id="sys_user_account" name="sys_user_account" placeholder="请写入合法的手机号" onblur="checkPhone()" class="inpt w200" maxlength="50" type="text"><span style="color:red">*&nbsp;</span><span id="cellPhone_unique" style="display:none;color:red;">手机号不能重复</span><span id="cellPhone_required" style="display:none;color:red;">手机号不能为空</span><span id="cellPhone_check" style="display:none;color:red;">手机号不符合规则</span><span id="cellPhone_maxlength" style="display:none;color:red;">手机号长度超长</span>						<span id="cellPhone_unique" style="display:none;color:red;">手机号不能重复</span>
	...
	//$( "#agree_open_bid" ).dialog({
		...
		//buttons: {
			//"同意": function() {					
				...
				//$("#nextBtn").attr("class","button-7 mr20");
				//$("#submitBtn").attr("class","button-7");
				//accepted();
			//},
			//"关闭": function() {
				//$( this ).dialog( "close" );
...
//     var certNo = jQuery.trim(jQuery("#certNo").val());
//     if(checkEncryptCertNo(certNo)) {
// 		  jQuery("#certNo").next().remove();
// 		  jQuery("#certNo").after("<span id='encryptCertNoInfo' style='color:red'>*该证件号已经注册，请<a href='"+appServer+"/custm/user/user_active.htm?certNo="+certNo+"'>认证</a>&nbsp;</span>");
// 		  jQuery("#certNo").focus();
// 		  return;
//    	} else {
// 	  jQuery("#encryptCertNoInfo").remove();
//    	}

// 		if(!doGroupSubmitCheck('add')) {
// 		return;
// 	}
// 		if($("#promoteMode").val() == "true"){
// 	if(!checkAgentCode()) {
// 		return false;
// 	}
// 	}
// 			    	    	    	    if(!doCheckPicCode()) { //图形验证码
//     		    return;
//     	    }
//     		                var mobile = jQuery.trim(jQuery("#cellPhone").val());
//           if(!checkEncryptMobile(mobile)) {
//               jQuery("#cellPhone").next().remove();
//               jQuery("#cellPhone").after("<span id='encryptMobileInfo' style='color:red'>*该手机号已经注册，请<a href='"+appServer+"/custm/user/check_mobile.htm?mobile="+mobile+"'>认证</a>&nbsp;</span>");
//               jQuery("#cellPhone").focus();
//               return;
//           } else {
//               jQuery("#encryptMobileInfo").remove();
//           }
//       	cptpd.submitForm('regForm');
