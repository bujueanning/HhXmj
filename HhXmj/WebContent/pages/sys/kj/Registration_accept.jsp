...
<div class="notice">
  ...
  <div class="zrxq-box-tqb">
    <form id="accept"action="bond/accept/save" method="post" enctype="multipart/form-data" onsubmit="return checkSubmit();">
    ...
    <div class="bd">
   		...
   		<c:forEach items="${varList}" var="var" varStatus="vs">
   			...
   			<input type="hidden" id="bond_company_name" name="bond_company_name" value="${var.bond_company_name }"/>
	        <input type="hidden" id="bond_asset_number" name="bond_asset_number" value="${var.bond_asset_number }"/>
   		</c:forEach>
    	<table class="zrxq-tb-2" width="100%">    		
         <tr>
            <th colspan="8" class="jbxq">请填写报名信息</th>
          </tr>
		   <tr>
            <th>处置方类型</th>
            <td>
           ...
           <input type="hidden" name="bond_accept_id" id="bond_accept_id" value="${bondAccept.bond_accept_id}"/>
     ...
     <div class="foot-action">
		...
		<input class="a-btn-sr" target="_blank" type='submit' value='提交报名信息'/>
    </div>
    </form>
  </div>
</div>
...
<script type="text/javascript">
	function showImg(){
		...
		f=$("#accept").find("#bond_deal_img").get(0).files[0]
		r.readAsDataURL(f);
		r.onload=function  (e) {
			$("#accept").find("#image").attr("src",this.result);
		}
	}
</script>
<script type="text/javascript">
