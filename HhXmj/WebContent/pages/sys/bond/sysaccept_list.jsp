    ...
    <div class="content-wrapper">
        ...
        <section class="content">
                            ...
                            <div class="box-body table-responsive no-padding">
                                <table class="table table-hover">
                                    <tr>
                                        ...
                                        <th style="word-break: keep-all;white-space:nowrap;width: 80px;">报名时间</th>
                                        ...
                                        <th style="word-break: keep-all;white-space:nowrap;width: 80px;">是否已完成订单</th>
            ...
            <c:forEach items="${varList}" var="var" varStatus="vs">
            	<tr>
            		 <td>
                         <input name='bond_accept_ids' id='bond_accept_ids'  type="checkbox" class="flat-red"
                            value="${var.bond_accept_id}">
                     </td>
					 <td>${var.bond_company_name}</a></td>
					 <td>${var.bond_asset_number}</td>
					 <td> <fmt:formatDate value="${var.bond_accept_time}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
					 <td>${var.bond_accept_name}</td>
					 <td>${var.bond_accept_property}</td>
					 <td>${var.bond_deal_name}</td>
					 <td>${var.bond_deal_phone}</td>
					 <c:if  test="${var.bond_choose==1}"><td  style="color:red;">未完成订单</td></c:if>
					 <c:if  test="${var.bond_choose==2}"><td>已完成订单</td></c:if>
					 <td>
                                                       
                                                        <shiro:hasPermission name="sysGoods:goedit">
                                                            <a style="cursor:pointer;" title="编辑"
                                                               onclick="edit('sys/bond/goaccept?bond_accept_id='+'${var.bond_accept_id}',800);"
                                                               class="btn btn-social-icon btn-info btn-xs"><i
                                                                    class="fa fa-edit"></i></a>
                                                        </shiro:hasPermission>
                                                        <shiro:hasPermission name="sysGoods:del">
                                                            <a style="cursor:pointer;" title="删除"
                                                               onclick="del('sys/bond/accdel?bond_accept_id='+'${var.bond_accept_id}',800);"
...
setTimeout(function(){
    ...
    $(function () {
        ...
        $('#checkAll').on('ifChecked', function (event) {
            $('input[name="bond_accept_ids"]').iCheck('check');
        });
        $('#checkAll').on('ifUnchecked', function (event) {
            $('input[name="bond_accept_ids"]').iCheck('uncheck');
        });
        $("#page").pagination({
