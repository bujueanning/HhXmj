...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    <section class="content">
      <div class="row">
        <div class="col-xs-12">
          <form class="form-horizontal" action="company/news/list" name="companynewsListForm" id="companynewsListForm" method="post">
        	  <input type="hidden" name="pageIndex" id="pageIndex" value="${companyNews.pageIndex}"/>
			  <input type="hidden" name="pageSize" id="pageSize" value="${companyNews.pageSize}"/>
			  <input type="hidden" name="total" id="total" value="${companyNews.total}"/>
	                ...
	                <div class="form-group">
	                  ...
	                  <div class="col-sm-3">
	                    <select id="conditionName" name="conditionName" class="form-control select2" >
									...
									<option value="company_news_id" <c:if test="${conditionName=='company_news_id'}"> selected </c:if> ></option>
									<option value="title" <c:if test="${conditionName=='title'}"> selected </c:if> >标题</option>
									<option value="company_news_createuid" <c:if test="${conditionName=='company_news_createuid'}"> selected </c:if> >创建者</option>
									<option value="company_news_createtime" <c:if test="${conditionName=='company_news_createtime'}"> selected </c:if> >创建时间</option>
									<option value="company_news_updatetime" <c:if test="${conditionName=='company_news_updatetime'}"> selected </c:if> >更新时间</option>
		                </select>
	                  </div>
	                ...
	                <c:choose>
							<c:when test="${not empty varList}">
								<c:forEach items="${varList}" var="var" varStatus="vs">
									<tr>
											<td>
												<input name='companyNewsIds' id='companyNewsIds' type="checkbox" class="flat-red" value="${var.company_news_id}" >
											</td>
										    <td>${var.title}</td>
										    <td>${var.sys_user_name}</td>
							              	<td><fmt:formatDate value="${var.company_news_createtime}" pattern="yyyy-MM-dd HH:mm:ss" /></td>
							              	<td><fmt:formatDate value="${var.company_news_updatetime}" pattern="yyyy-MM-dd HH:mm:ss" /></td>
											<td>
												<shiro:hasPermission name="sysUser:goedit">
													<a style="cursor:pointer;" title="编辑" onclick="edit('company/news/goedit?company_news_id='+'${var.company_news_id}',800);" class="btn btn-social-icon btn-info btn-xs"><i class="fa fa-edit"></i></a>
												</shiro:hasPermission>
												<shiro:hasPermission name="sysUser:del">
													<a style="cursor:pointer;" title="删除" onclick="del('company/news/del?company_news_id='+'${var.company_news_id}',800);" class="btn btn-social-icon btn-primary btn-xs"><i class="fa fa-trash"></i></a>
												</shiro:hasPermission>
											</td>
									</tr>
								</c:forEach>
	            ...
	            <div class="box-footer clearfix">
	            	<div class="pull-left">
	            		<shiro:hasPermission name="sysUser:goadd">
	            			 <a style="cursor:pointer;" title="新增" onclick="add('company/news/goadd',800);" class="btn btn-social-icon btn-info"><i class="fa fa-plus-square"></i></a>
	            		</shiro:hasPermission>
	            		<shiro:hasPermission name="sysUser:delall">
	            			<a style="cursor:pointer;" title="删除" onclick="delArray('company/news/delall',800);" class="btn btn-social-icon btn-primary"><i class="fa fa-trash"></i></a>
	            		</shiro:hasPermission>
	            	</div>
  ...
  $(function () {
	...
	//Flat red color scheme for iCheck
	 ...
	 $('#checkAll').on('ifChecked', function(event){
	 		$('input#companyNewsIds').iCheck('check');
	 	});
	 	$('#checkAll').on('ifUnchecked', function(event){
	 		$('input#companyNewsIds').iCheck('uncheck');
	 	});
	 $("#page").pagination({
    ...
    function delArray(url,height){
 		var companyNewsIds = new Array();
 		 $("#companyNewsIds:checked").each(function() {
 			companyNewsIds.push($(this).val()); 
 		 });
    	window.top.delArrayDialog(url,companyNewsIds);
    }
    function postObjs(url,height){
 		var objs = new Array();
 		 $("#companyNewsIds:checked").each(function() {
 			var obj = {};
 			obj.companyNews_id = $(this).val();
 			objs.push(obj); 
 		 });
